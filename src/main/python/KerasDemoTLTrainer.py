...
def parse_arguments():
    ...
    parser.add_argument("--tdata", type=str)
...
if "__main__"==__name__:
    ...
    train_data = read_data(args.tdata)
    test_data = read_data(args.data)

    train_links = train_data[["hypothesis", "evidence"]].values
    train_labels = train_data["label"].values

    train_labels = train_labels == "link"
    ...
    model = create_model([50], 0.001, hypothesis_max_length, sentence_max_length, vectorizer.embeddings, args.seed)
    ...
    model.fit({"hypothesis_input":vectorized_hypotheses, "evidence_input": vectorized_sentences}, two_d_train_labels, epochs=args.epochs, verbose=True)
    # model.fit({"hypothesis_input":vectorized_hypotheses[::10], "evidence_input": vectorized_sentences[::10]}, two_d_train_labels[::10], epochs=1, verbose=True)

    test_links = test_data[["hypothesis", "evidence"]].values
    test_labels = test_data["label"].values == "link"
    two_d_test_labels = np.zeros((test_labels.shape[0], 2))
    two_d_test_labels[np.where(test_labels==0), 0] = 1
    two_d_test_labels[np.where(test_labels==1), 1] = 1
    ...
    model.fit({"hypothesis_input":test_vectorized_hypotheses, "evidence_input": test_vectorized_sentences}, two_d_test_labels, epochs=2, verbose=True)
    ...
    with open("../models/hypothesisevidencelinking-tl-demo.json", "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save("../models/hypothesisevidencelinking-tl-demo.h5")
