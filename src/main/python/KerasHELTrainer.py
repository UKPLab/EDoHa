...
def read_data(data_path):
    data_files = sorted(glob.glob("{0}/*-expanded.csv".format(data_path)))
    data = pd.concat([pd.read_csv(f, sep=",", quotechar="'", header=0, index_col=0) for f in data_files]) 
    return data
...
if "__main__"==__name__:
    ...
    test_user = "user0"
    test_group = user_groups.get_group(test_user)

    train_links = list()
    train_labels = list()
    for name, group in user_groups:
        if name == test_user: continue
    ...
    with open("../models/hypothesisevidencelinking.json", "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save("../models/hypothesisevidencelinking.h5")
